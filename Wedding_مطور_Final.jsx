...
// ╔══════════════════════════════════════════════════════════════╗
// ║         سكربت معالج قوالب الأفراح - النسخة المنظمة          ║
// ║         Wedding Batch Script - ESL Final (Organized)         ║
// ╚══════════════════════════════════════════════════════════════╝

function weddingBatchScript_ESL_Final() {

    // #region ══════════════════════════════════════════════════════
    // #region  [1] الإعدادات والمتغيرات العامة | Settings & Variables
    // #region ══════════════════════════════════════════════════════

    // متغيرات تخزين الملفات المختارة من المستخدم
    var selectedCalligraphyFile = null;  // ملف المخطوطة (M1)
    var selectedCharacterFiles  = [];    // صور الشخصيات (S1)
    var selectedShapeFiles      = [];    // صور الأشكال (S2, S3)
    var selectedPsdFiles        = [];    // ملفات القوالب PSD
    var selectedTxt             = null;  // ملف البيانات TXT
    var selectedLogoFile        = null;  // شعار العميل

    // متغيرات الحالة أثناء التشغيل
    var fileContentGlobal    = "";   // محتوى ملف النصوص كاملاً
    var customSavePath       = null; // مسار الحفظ المخصص (اختياري)
    var globalShapeImageIndex = 0;   // مؤشر دوران صور الأشكال

    // #endregion ═══════════════════════════════════════════════════
    // #endregion  [1] نهاية قسم الإعدادات والمتغيرات
    // #endregion ═══════════════════════════════════════════════════


    // #region ══════════════════════════════════════════════════════
    // #region  [2] واجهة المستخدم | UI Construction
    // #region ══════════════════════════════════════════════════════
    ...
    // صندوق البيانات الأساسية المستخرجة من الملف (للمراجعة والتعديل)
    ...
    basicDataPanel.enabled       = false; // يُفعَّل تلقائياً عند تحميل ملف TXT
    ...
    // مجموعة تفاصيل الحفظ (مخفية حتى يُفعَّل الحفظ)
    ...
    // ── العمود الأيمن (الملفات والملحقات) ────────────────────────
    ...
    // #endregion ═══════════════════════════════════════════════════
    // #endregion  [2] نهاية قسم واجهة المستخدم
    // #endregion ═══════════════════════════════════════════════════


    // #region ══════════════════════════════════════════════════════
    // #region  [3] الدوال المساعدة | Helper Functions
    // #region ══════════════════════════════════════════════════════

    /**
     * البحث عن طبقة بالاسم بشكل تعاودي داخل المجموعات
     * @param {Object} parent - الوثيقة أو المجموعة للبحث داخلها
     * @param {String} layerName - اسم الطبقة المطلوبة
     * @returns {Layer|null} الطبقة إذا وُجدت، أو null
     */
    ...
    /**
     * تنسيق نص المهنئين بحماية الأسماء من الانكسار في منتصفها
     * يستبدل المسافات داخل الاسم الواحد بـ Non-Breaking Space
     * @param {String} text - النص الخام للمهنئين
    ...
    /**
     * توزيع النص الإنجليزي بذكاء بناءً على عدد كلمات القالب الأصلي
     * يختصر أو يوزع الكلمات حسب المساحة المتاحة في الطبقة
     * @param {String} originalText - النص الأصلي في القالب
     * @param {String} newText - الاسم الإنجليزي الجديد
     * @returns {String} النص الموزع
     */
    ...
    /**
     * توزيع النص العربي ذكياً بالحفاظ على هيكل الأسطر الأصلية في الطبقة
     * يحسب عدد الكلمات في كل سطر أصلي ويعيد التوزيع على نفس البنية
     * @param {String} originalText - النص الأصلي متعدد الأسطر
     * @param {String} newText - النص الجديد
     * @returns {String} النص الموزع على أسطر
     */
    function getSmartDistributedText(originalText, newText) {
        ...
        for (var k = 0; k < originalStructure.length; k++) {
            ...
            // إضافة الكلمات الزائدة لآخر سطر
    ...
    /**
     * بناء نص X1 (الاسم الكامل) بذكاء بحسب عدد كلمات القالب
     * يختار الاسم الأول + عدد مناسب من كلمات بقية الاسم
     * @param {String} originalText - النص الأصلي في طبقة X1
     * @param {String} firstName - الاسم الأول
     * @param {String} restName  - بقية الاسم
     * @returns {String} الاسم الكامل الذكي
     */
    function getSmartX1Text(originalText, firstName, restName) {
        ...
        var resultWords = [];
        if (fName !== "") resultWords.push(fName);
        ...
        if (originalCount === 2) {
            ...
            for (var i = 0; i < restCount; i++) resultWords.push(restWords[i]);
        }
    ...
    // #endregion ═══════════════════════════════════════════════════
    // #endregion  [3] نهاية قسم الدوال المساعدة
    // #endregion ═══════════════════════════════════════════════════


    // #region ══════════════════════════════════════════════════════
    // #region  [4] أحداث الأزرار | Button Events
    // #region ══════════════════════════════════════════════════════
    ...
    btnTxt.onClick = function() {
        ...
        if (selectedTxt) {
            ...
            // استخراج البيانات الأساسية بالتعبيرات النمطية
            ...
            // التسمية التلقائية للمجلد والملفات بناءً على اسم العريس
            var autoName = "العريس";
    ...
    // ── تبديل ظهور تفاصيل الحفظ عند تفعيل الخيار ────────────────
    ...
    // #endregion ═══════════════════════════════════════════════════
    // #endregion  [4] نهاية قسم أحداث الأزرار
    // #endregion ═══════════════════════════════════════════════════


    // #region ══════════════════════════════════════════════════════
    // #region  [5] المحرك الأساسي | Core Logic
    // #region ══════════════════════════════════════════════════════

    /**
     * الدالة الرئيسية للمعالجة الآلية - تُشغَّل عند الضغط على "بدء المعالجة"
     * تمر على كل ملف PSD بالترتيب وتُطبق:
     *   - استبدال المخطوطة (M1)
     *   - استبدال صور الشخصيات (S1)
     *   - حشو صور الأشكال (S2, S3)
     *   - تحديث طبقات النصوص (A1, A2, X1, X2, X3, X4)
     *   - إضافة الشعار
     *   - الحفظ كـ PSD و/أو JPG
     */
    applyBtn.onClick = function() {

        // ── التحقق من اختيار الملفات الأساسية ────────────────────
        ...
        // #region ── نافذة التقدم Progress Window ──────────────────
        ...
        progressWin.center();
        progressWin.show();
        // #endregion ── نهاية نافذة التقدم ────────────────────────

        // ── تهيئة متغيرات الدُفعة ────────────────────────────────
        var successCount   = 0;
        var failCount      = 0;
        ...
        // بناء نص "أفراح آل ..." من آخر كلمة في بقية الاسم
        var afrahText = "";
        if (restName !== "") {
        ...
        // ════════════════════════════════════════════════════════
        // ══ حلقة المعالجة الرئيسية (ملف PSD واحد في كل دورة) ══
        // ════════════════════════════════════════════════════════
        for (var i = 0; i < selectedPsdFiles.length; i++) {
            ...
            // استخراج نص المهنئين الخاص بهذا التصميم من الملف النصي
            var num        = i + 1;
            ...
            try {
                ...
                if (oldCalligraphyLayer !== null) {
                    if (selectedCalligraphyFile !== null) {
                        ...
                        if (isJpg) {
                            // JPG: تحديد قناة RGB ونسخ المحتوى
                            ...
                            // PNG: نسخ المحتوى كاملاً مع الشفافية
                        ...
                        // لصق الطبقة الجديدة فوق الطبقة القديمة
                        doc.activeLayer = oldCalligraphyLayer;
                        var newLayer    = doc.paste();

                        // حساب نسبة التكبير/التصغير للحفاظ على أبعاد الإطار
                        ...
                        // محاذاة الطبقة الجديدة مع موقع الإطار القديم
                        ...
                        // نقل التأثيرات (Layer FX) إن وُجدت
                        doc.activeLayer = oldCalligraphyLayer;
                        var hasStyle    = false;
                        try {
                        ...
                        // تنظيف الطبقات القديمة وإعادة التسمية
                        oldCalligraphyLayer.remove();
                        newLayer.name = "M1";

                        // حذف X2 و X1 عند وجود مخطوطة (لأن المخطوطة تحل محلهما)
                ...
                // #endregion ── نهاية معالجة M1 ───────────────────

                // #region ── معالجة صور الشخصيات (S1) ─────────────
                var oldS1Layer = findLayerByName(doc, "S1");

                if (oldS1Layer !== null) {
                    if (selectedCharacterFiles.length > 0) {
                        // توزيع الصور بالتناوب على القوالب
                ...
                // #endregion ── نهاية معالجة S1 ───────────────────

                // #region ── معالجة صور الأشكال (S2, S3) ──────────
                ...
                for (var s = 0; s < shapeLayers.length; s++) {
                    ...
                    if (shapeLayer !== null && selectedShapeFiles.length > 0) {
                        ...
                        // حذف الطبقات المرتبطة (Clipping Masks) القديمة فوق الشكل
                        while (keepChecking) {
                            keepChecking = false;
                            var layerIndex = -1;
                        ...
                        // اختيار صورة الشكل بالتناوب العالمي
                        ...
                        newImgLayer.grouped = true; // ربط بـ Clipping Mask مع الشكل

                        // تكبير الصورة لملء الإطار بالكامل (Cover)
                ...
                // #endregion ── نهاية معالجة S2, S3 ───────────────

                // #region ── تحديث طبقات النصوص (A1,A2,X1,X2,X3,X4) ──
                    ...
                    if (lyr !== null && lyr.kind === LayerKind.TEXT) {
                        ...
                        // X4: المهنئون - خوارزمية Fit النص الذكية
                            ...
                            try {
                                ...
                                // دالة داخلية: هل النص يتسع في الإطار؟
                                function fits() {
                                    var b = lyr.bounds;
                                ...
                                // مرحلة 1: Fast Shrink (تصغير سريع بخطوات 4pt)
                                ...
                                // مرحلة 2: Binary Search (ضبط دقيق للحجم الأمثل)
                                var low  = 5;
                                ...
                                // تنظيف: إزالة الشرطات الزائدة في بداية/نهاية كل سطر
                ...
                // #endregion ── نهاية تحديث طبقات النصوص ──────────

                // #region ── إضافة الشعار ──────────────────────────
                if (selectedLogoFile !== null) {
                    var logoPosition = "right"; // يمكن تغييره إلى "left"
                    ...
                    // الحجم الأقصى = 15px لكل 300px من العرض
                    ...
                    // تكبير/تصغير بحفاظ على النسبة
                    var scaleRatio = (lW > lH)
                        ? (maxPx / lW) * 100
                        : (maxPx / lH) * 100;
                    ...
                    var targetX = (logoPosition === "right")
                        ? docW - nlW - marginPx
                        : marginPx;

                    logoLayer.translate(targetX - nlX, targetY - nlY);
                    app.preferences.rulerUnits = originalRulerUnits;
                }
                // #endregion ── نهاية إضافة الشعار ─────────────────

                // #region ── حفظ الملف (PSD و/أو JPG) ─────────────
                if (chkSave.value === true) {
                    // إنشاء مجلد الحفظ عند أول ملف فقط
                    if (batchOutFolder === null) {
                        var baseFolder      = customSavePath ? customSavePath : selectedPsdFiles[0].parent;
                        var folderNameText  = txtFolderName.text || "المخرجات النهائية";
                        batchOutFolder      = new Folder(baseFolder + "/" + folderNameText);
                        var folderCounter   = 1;

                        // تجنب الكتابة فوق مجلد موجود
                    ...
                    var fileNameText   = txtFileName.text || "تصميم";
                    var suffix         = (i === 0) ? "" : " " + (i + 1);
        ...
        } // ══ نهاية حلقة المعالجة الرئيسية ══

        progressWin.close();

        // #region ── نافذة ملخص النتائج ────────────────────────────
        ...
        if (failCount > 0) {
            summaryPanel.add("statictext", undefined, "قوالب فشل معالجتها: " + failCount);
        }
        ...
        if (warningMessages.length > 0) {
            ...
            for (var w = 0; w < warningMessages.length; w++) {
                ...
                try {
                    warnText.graphics.foregroundColor = warnText.graphics.newPen(
                        warnText.graphics.PenType.SOLID_COLOR, [0.8, 0, 0, 1], 1
                    );
                } catch(err) {}
            }
        }
        ...
        btnCloseEnd.onClick = function() { endWin.close(); };

        endWin.show();
        // #endregion ── نهاية نافذة ملخص النتائج ──────────────────

    }; // نهاية applyBtn.onClick

    // #endregion ═══════════════════════════════════════════════════
    // #endregion  [5] نهاية قسم المحرك الأساسي
    // #endregion ═══════════════════════════════════════════════════


    // #region ══════════════════════════════════════════════════════
    // #region  [6] تشغيل السكربت | Execution
    // #region ══════════════════════════════════════════════════════

    // إظهار النافذة الرئيسية وتشغيل السكربت
    win.show();

    // #endregion ═══════════════════════════════════════════════════
    // #endregion  [6] نهاية قسم التشغيل
    // #endregion ═══════════════════════════════════════════════════
